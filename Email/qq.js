RSA = function () {
    ...
    var pt;
    "Microsoft Internet Explorer" == "Netscape" ? (r.prototype.am = l,
        pt = 30) : "Netscape" != "Netscape" ? (r.prototype.am = a,
    ...
    if (null == ut) {
        ut = new Array,
            gt = 0;
        var dt;
        //         if ("Netscape" == "Netscape" && "5.0 (Macintosh; Intel Mac OS X 10_14_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.121 Safari/537.36" < "5" && window.crypto && window.crypto.random) {
...
function getPassword(username, mima, verifycode) {
    var password = Encryption.getEncryption(mima, Encryption.salt(username), verifycode, undefined);
    return password
}
...
function getAction() {
    var action_list = [2,24]
    var action = action_list.join("-") + "-" + (new Date - 0);
    return action
